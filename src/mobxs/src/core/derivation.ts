...
export function checkIfStateModificationsAreAllowed(atom: IAtom) {
    ...
    const hasObservers = atom.observers_.size > 0;//atom.observers_是Set结合存储Reaction当没有Reaction允许直接修改当有时必须在action下修改
    //action修改了globalState.allowStateChanges值
...
export function trackDerivedFunction<T>(derivation: IDerivation, f: () => T, context: any) {
    ...
    changeDependenciesStateTo0(derivation)//derivation为Reaction
    derivation.newObserving_ = new Array(derivation.observing_.length + 100)
    derivation.unboundDepsCount_ = 0
    derivation.runId_ = ++globalState.runId
    const prevTracking = globalState.trackingDerivation
    globalState.trackingDerivation = derivation//设置此值用来执行变量get时调用reportObserved方法是将derivation里newObserving_存入正在读取的ObservableValue
    globalState.inBatch++
    let result
...
function bindDependencies(derivation: IDerivation) {
    ...
    while (l--) {
        ...
        console.log('在删除observer逻辑中复用那些仍响应的observer')
        dep.diffValue_ = 0
    }
...
export function clearObserving(derivation: IDerivation) {
    ...
    const obs = derivation.observing_;console.error('清除')

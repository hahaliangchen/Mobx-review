...
export class ObservableObjectAdministration
    ...
    setObservablePropValue_(key: PropertyKey, newValue): boolean | null {
        ...
        if (newValue !== globalState.UNCHANGED) {
            ...
            console.log(this.target_.value,'调用observable执行完set但又读了一下',this)
    ...
    make_(key: PropertyKey, annotation: Annotation | boolean): void {
        ...
        while (source && source !== objectPrototype) {
            const descriptor = getDescriptor(source, key)//得到此属性的描述符，比如可读可写可枚举
            if (descriptor) {
    ...
    defineObservableProperty_(
        key: PropertyKey,
        value: any,//observable修饰属性的值(descriptor提供),这里是我Doubler对象里的value属性值为0
        ...
        try {
            ...
            console.log(this.target_,cachedDescriptor,value,this.target_.value,'定义观察属性')
            // Define
            if (proxyTrap) {
                if (!Reflect.defineProperty(this.target_, key, descriptor)) {
                    return false
...
export function asObservableObject(
    ...
    //将$mobx(Symbol(mobx administration))属性添加到target(用户自定义对象),并赋值成adm(ObservableObjectAdministration类型)
...
function getCachedObservablePropDescriptor(key) {
    ...
    //this是用户自定义的对象,this[$mobx]是ObservableObjectAdministration对象内部value_是Map存储this的一些被annotation修饰过的key，以及相对应的对象如ObservableValue
    return (
        descriptorCache[key] ||
        (descriptorCache[key] = {
            get() {
